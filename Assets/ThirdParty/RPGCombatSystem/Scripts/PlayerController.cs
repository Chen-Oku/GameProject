...
public class PlayerController : MonoBehaviour
{
    ...
    public CameraOrbit cameraOrbit; //Camera orbit script
    ...
    private bool canDoubleJump = true; // Permitir doble salto cuando el jugador toca el suelo
    ...
    void Update()
    {
        ...
        */
        if (charCont.isGrounded)
        {
            ...
            {
                canJump = true;
                canDoubleJump = true; // Permitir doble salto cuando el jugador toca el suelo
                anim.SetBool("Jump", false);
                if (fallTime > 0.2f)
                {
                    soundMan.PlaySound("Land");
        ...
        // Actualizar la rotación del personaje para que coincida con la rotación de la cámara
        if (cameraOrbit != null)
        {
            Vector3 cameraForward = cameraOrbit.transform.forward;
            cameraForward.y = 0; // Mantener solo la rotación en el plano XZ
